...
function Planets(props) {
    let planet;
    // * Controls the menu bar for the planets
    let [menuOpen, setMenuOpen] = useState(false);

    // * If there's a specific planet in the param it shows its info, otherwise its set to earth by default
    ...
    let handleCLick = () => setMenuOpen(!menuOpen);
        ...
        <section className={`planets-page ${name}`}>
            <div className='planets-card'>
                ...
                <nav style={{ backgroundColor: color }}>
                    ...
                    <header className='planets-header' onClick={handleCLick}>
                        <h2>{name}</h2>
                        <FontAwesomeIcon className={`planets-menu-icon ${menuOpen ? 'rotate-icon ' : undefined}`} icon={faCaretDown} />
                    </header>
